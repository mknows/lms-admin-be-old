	...
	index: async (req, res) => {
		try {
			...
			getNameFile.forEach((file) => {
				valueLink.push(file.dataValues.image);
			});

			let finalFileLink = valueLink.forEach(async (file) => {
				await getDownloadURL(ref(storage, file));
			});

			console.log("finalFileLink => ", finalFileLink);
			...
			return res.sendJson(200, true, "success get all data", valueLink);
		} catch (error) {
			console.log(error);
